...
def test_inconsistent_type_object_last(value, infix, empty_merger):
    ...
    assert (
        str(excinfo.value)
        == f"An earlier release had the value {infix} for /integer, but the current release has an object with a 'object' key"  # noqa: E501
    )
...
def test_inconsistent_type_object_first(empty_merger):
    ...
    assert (
        str(excinfo.value)
        == "An earlier release had the object {'object': 1} for /integer, but the current release has an array"
    )
